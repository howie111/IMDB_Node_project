...
app.get('/', (req, res) => {
    ...
    ejs only accept the object 
    -------------------------------------------------- */
    var movieObj = getmovies();
    //console.log(movieObj);
    res.render('pages/index',{movieObj});
})
app.get('/movie/:movieId', (req, res) => {
   ...
   for(var i =0;i<movieObj.length;i++){
     if(movieObj[i].id==req.params.movieId)
     movieItem =movieObj[i]; 
   }
    res.render('pages/movie',{movieItem})
})
/*** locally search movie ***/
app.post('/search',(req,res)=>{
 var bytitle = req.body.test.toLowerCase();
 console.log(bytitle);
 var movieslists = [];
 var error = null;
 var movieObj = getmovies();
 for(let i = 0;i<movieObj.length;i++){
   if(movieObj[i].title.toLowerCase().includes(bytitle)){
     movieslists.push(movieObj[i]);
   }
   else{}
 }
 ...
 res.render('pages/result',{movieslists, error})
})

// search movies from omdbAPI
app.post('/apisearch',(req,res)=>{

 //var bytitle = req.body.bytitle.toLowerCase();
 var movieslists = {};
 var error = null;
 var bytitle = req.body.test.toLowerCase();
 console.log('i am here' +bytitle)
 var url = 'http://www.omdbapi.com/?s='+ bytitle+'&type=movie';
  http.get(url,(response)=>{
   ...
   response.on('end',()=>{
      movieslists = JSON.parse(body)
      //console.log(movieslists);
     if (movieslists.Response == 'False') {
       error = true;
     }
     console.log(movieslists)
     res.render('pages/apiresult',{movieslists, error}) 
   })
 })
})

/**** view the specific movie page ***/
app.get('/apimovie/:title',(req,res)=>{
  var movieItem={};
  let title = req.params.title; 
  var url = 'http://www.omdbapi.com/?t='+title;
  var rmUrl ='https://www.rottentomatoes.com/m/'+title.split(' ').join('_');
  console.log(rmUrl);
  http.get(url,(response)=>{
   let body ="";
   response.on('data',(chunk)=>{
   ...
   response.on('end',()=>{
      movieItem = JSON.parse(body)
      //movieItem.RMrating =scape(rmUrl);
      //var a = scape(rmUrl);
      request(rmUrl,(err,response,body)=>{        
          if(!err){
            var rating=[];
            var $=cheerio.load(body);
            console.log(response.statusCode)
            if(response.statusCode === 404){
              movieItem.RTrating = "N/A"; 
              res.render('pages/apimovie',{movieItem});
              
            }
           
            movieItem.trailerUrl = $('.trailer_play_action_button').attr('data-mp4-url');
            $('div.active').find('a > span').each(function(i,elem){
              rating[i] = $(this).text() === ''? null : $(this).text();
              if(rating[1]){ 
                movieItem.RTrating = rating[1]; 
                console.log("test of rating"+rating[1]);
                res.render('pages/apimovie',{movieItem});
              }
            else{
                 // res.render('pages/apimovie',{movieItem}); 
